...
export const getLearnedDetailTool = tool(
  ...
  },
  {
    ...
    description:
      "读取指定缓存方案的完整详细信息。传入索引号（数字）或 id（uuid 字符串）来获取方案的完整内容。",
    schema: z.object({
...
export const updateLearnContentTool = tool(
  ...
  },
  {
    ...
    description:
      "更新已有缓存方案的内容。传入缓存项的 id（uuid 字符串）和新的 Markdown 内容，覆盖原有文件。适用于补充更多细节或修正之前的方案。",
    schema: z.object({
 ...
 */
export const getCatalogFilePathTool = tool(
  () => {
