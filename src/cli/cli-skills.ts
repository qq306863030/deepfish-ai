...
export function registerSkillsCommands(program: Command) {
  ...
  skills.command('enable <indices...>').description('按索引启用技能，多个用空格或逗号分隔').action(handleSkillsEnable);
  skills.command('use <indices...>').description('按索引启用技能（同 enable）').action(handleSkillsEnable);
  skills.command('disable <indices...>').description('按索引禁用技能，多个用空格或逗号分隔').action(handleSkillsDisable);
