...
import { testServer, getAgentId, initSession } from '../cli-utils/init-agent';
import { getServePort } from '../cli-utils/getGlobalData';
import { logError, logErrorMsg } from '@/client/cli-utils/print';
...
/** 处理服务端转发的日志消息 */
function handleLogMessage(msg: { payload: { message: string; color?: string } }) {
  const { message, color } = msg.payload;
  if (color) {
...
/** 处理服务端转发的行内消息 */
function handleWriteLineMessage(msg: { payload: { message: string; color?: string } }) {
  const { message, color } = msg.payload;
  if (color) {
  ...
  try {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      ws.on('message', (data) => {
        ...
        if (msg.type === 'writeLine') {
          handleWriteLineMessage(msg);
          return;
        }
