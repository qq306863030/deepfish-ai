...
function readTaskList(taskListPath) {
    return fs.readFileSync(taskListPath || path.join(process.cwd(), `tmp_tasklist_${this.agentRobot.id}.json`), { encoding: 'utf-8' }) || '[]'
}

// 更新任务列表
function updateTaskList(taskListPath, list) {
    try {
        fs.writeFileSync(taskListPath || path.join(process.cwd(), `tmp_tasklist_${this.agentRobot.id}.json`), JSON.stringify(list, null, 2), { encoding: 'utf-8' })
        return true
    } catch (error) {
        ...
        function: {
            ...
            parameters: {
                ...
                properties: {
                    taskListPath: {
                        type: 'string',
                        description: '任务列表文件路径，默认值为当前目录下tmp_tasklist_「你的编号」.json',
                    },
                },
